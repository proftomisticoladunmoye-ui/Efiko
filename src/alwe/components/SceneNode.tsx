    ...
    <>
      ...
      {showCheck && scene.knowledgeCheck && (
        <div className="alwe-kc">
          ...
          <MiniCheck
            check={scene.knowledgeCheck}
            onResult={(c, tags) => { onCheckResult?.(scene.id, c, tags); if (!c) setMissed(true); }}
          />
          {missed && onReplayScene && (
            <ExplainMistake
              items={[{ sceneId: scene.id, sceneTitle: scene.title, commonMistakes: scene.explain.commonMistakes, simpler: scene.explain.simpler, concepts: [] }]}
